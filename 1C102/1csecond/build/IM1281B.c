...
//extern int receive_finished;//指示数据是否完成

int i = 8;
int j = 0;

int reveive_numbe = 0;
//uint16_t USART1_RX_STA = 0;//记录接受数据状态
extern uint8_t receive_index;

//USART1_IRQHandler():
//这是USART3的中断服务函数。当USART接收到数据时
//触发USART_IT_RXNE中断，函数会读取接收到的数据，并将其存储到Rx_Buffer1数组中。
//如果检测到USART_IT_IDLE中断，表示接收完毕，receive_finished标志位会被设置为1
...
DataArray Analysis_data(void)
{
	...
	//{
		...
		reveive_numbe = receive_index; //这个也是好像写的
		//receive_finished = 0;
