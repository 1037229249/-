...
void UART2_INT(void)
{
    ...
    //receive_Data[0]=UART_ReceiveData(UART2); //鎺ユ敹鏁版嵁
    //Uart2_SendData(receive_Data);       //灏嗘帴鏀跺埌鐨勬暟鎹粠涓插彛2鍙戦�佸嚭鍘?
    //Queue_Wirte(&Circular_queue, (uint8_t *)&receive_Data, 1); //鍚戦槦鍒椾腑鍐欏叆鏁版嵁
...
void intc_handler(void)
{
    ...
    if (IntReg & UART0_INT_OUT) //Uart0
    {
        ...
        if(UART0_IIR&0x04)
        {
            while(UART0_LSR&0x1)
            {
                receive_Data[0]=UART_ReceiveData(UART0); //接收数据
    ...
    if (IntReg & UART1_INT_OUT) //Uart1
    {
        ...
        if(UART1_IIR&0x04)
        {
            while(UART1_LSR&0x1)
            {
                receive_Data1[0]=UART_ReceiveData(UART1); //接收数据
                Rx_Buffer1[receive_index++]= receive_Data1[0];
                if(receive_index>MAX_receive)
                {
                receive_index=MAX_receive;
                }
                //Queue_Wirte(&Circular_queue_UART1, (uint8_t *)&receive_Data1, 1); //向队列中写入数据
